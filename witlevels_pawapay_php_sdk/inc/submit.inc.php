...
if (isset($_POST['submit_btn'])) {
  ...
  if (!empty($pay['error'])) {
    echo 'Payment request error: ' . htmlspecialchars($pay['error']);
    exit();
  }

  if ($pay['http_code'] < 200 || $pay['http_code'] >= 300) {
    echo 'Payment request failed. HTTP code: ' . (int) $pay['http_code'];
    exit();
  }

  $paymentStatus = '';
  if (is_object($pay['result']) && isset($pay['result']->status)) {
    $paymentStatus = strtoupper((string) $pay['result']->status);
  }

  if ($paymentStatus === 'REJECTED') {
    echo 'Transaction was rejected by provider.';
    exit();
  }
...
<script>
  ...
  function checkStatus() {
    ...
    xhr.open('GET', '../verify.php?token=<?php echo urlencode($pay['deposit_id']); ?>');
  ...
  function handleStatus(status) {
    if (status === 'success') {
      ...
      setTimeout(function() {
        window.location.href = '../success.php?token=<?php echo urlencode($pay['deposit_id']); ?>'; // replace with the URL you want to redirect to
